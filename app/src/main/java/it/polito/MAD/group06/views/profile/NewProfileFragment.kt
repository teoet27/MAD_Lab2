            ...
            override fun handleOnBackPressed() {
                userProfileViewModel.currentUser.observe(viewLifecycleOwner) { user ->
                    if (editFullNameOBJ.text.toString() == "") {
                        Snackbar.make(
                            requireView(), "Error: you must provide your full name. Try again.", Snackbar.LENGTH_LONG
                        ).show()
                    } else if (editNicknameOBJ.text.toString() == "") { //TODO: check if nickname is already present in database (it must be unique)
                        Snackbar.make(
                            requireView(), "Error: you must provide a nickname. Try again.", Snackbar.LENGTH_LONG
                        ).show()
                    } else if (editLocationOBJ.text.toString() == "") {
                        Snackbar.make(
                            requireView(), "Error: you must provide your location. Try again.", Snackbar.LENGTH_LONG
                        ).show()
                    } else if (editPhoneOBJ.text.toString() == "") {
                        Snackbar.make(
                            requireView(), "Error: you must provide your phone number. Try again.", Snackbar.LENGTH_LONG
                        ).show()
                    } else {
                        saveData()
